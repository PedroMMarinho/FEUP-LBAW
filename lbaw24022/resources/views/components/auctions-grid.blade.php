<div class="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-4 justify-items-center">
    @foreach ($auctions as $auction)
        <x-auction 
            ...
            :highest-bid="$auction->bids->isEmpty() ? $auction->minimum_bid : $auction->bids[0]->value"
            :image="$auction->images->isEmpty() ? null : $auction->getImages()[0]"
            :following="Auth::Check() ? $auction->followers->contains('id', Auth::id()) : false"
            :seller="$auction->seller->id"
            :state="$auction->auction_state"
            :delivery-location="$auction->delivery_location"
            :winner="$auction->bids->isEmpty() ? null : $auction->bids[0]->user" />
    @endforeach
