...
<section>
    ...
    <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
        <p class="mb-3">At least 2 images</p>
        <div id="photoWrapper" class="flex gap-6 flex-wrap mx-auto flex-shrink">
            <div id="photoUploadButton" class="size-36 bg-gray-100 flex items-center justify-center cursor-pointer">
                <input id="tempPhotoInput" class="hidden read-only:" type="file" accept="image/png image/jpg image/jpeg" multiple name="temp">
                <span class="w-max px-auto border-b-2 border-transparent hover:border-gray-800">Add Photo</span>
            </div>
        </div>
        <div class="flex-end mt-4">

        @php
            $firstPhotoError = collect($errors->get('photos.*'))
                ->flatten()
                ->first();
            $errorNumber = $errors->get('photos');
        @endphp

        <x-input-error class="mt-2" :messages="$firstPhotoError ? [$firstPhotoError] : $errorNumber" />
        <p id="photoCounter" class="text-end  ml-7">0/10 photos added</p>
...
function createPhotoInput(image, url, newPhoto)
{
    ...
    wrapper.id ="";
    wrapper.classList.add("relative","size-36","min-w-36");
    
    // Create img wrapper
    const imgWrapper = document.createElement("div");
    imgWrapper.classList.add("absolute","inset-0","overflow-hidden");
    ...
    deleteWrapper.append(svgWrapper);
    titleWrapper.append(fileNameTitle);
    overlayWrapper.append(titleWrapper);
    overlayWrapper.append(deleteWrapper);
    ...
    wrapper.append(overlayWrapper);

    // Create logic for delete button
    svgWrapper.addEventListener("click", () => {
        photoWrapper.removeChild(wrapper);
        uploadButton.style.display ="flex";
        addedPhotos -= 1;
        counter.textContent = `${addedPhotos}/${maximumPhotos} photos added`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Create counter
    counter.textContent = `0/${maximumPhotos} photos added`

    // Put photos previews already stored

    window.images.forEach(imageUrl => {
        addedPhotos += 1;
        photoWrapper.insertBefore(createPhotoInput(null, imageUrl, false), uploadButton);
    });

    if (addedPhotos >= maximumPhotos) uploadButton.style.display = "none";

    // Update variables


    // Create event listener for the tempInput
    ...
    tempInput.addEventListener("change", (e)=>{
        ...
        for (i = 0; i < e.target.files.length; i++) {
            ...
            reader.onload = () => {
                ...
                photoWrapper.insertBefore(createPhotoInput(image, reader.result, true), uploadButton);
        ...
        if (ignoredFiles) {
            console.log(ignoredFiles);
            // Show somewhere the ingored files
            showPopUp(ignoredFiles +  (ignoredFiles == 1 ? ' file' : ' files') + ' ignored. Image cannot exceed 2MB', 'error');
        }
        tempInput.value = "";
    })
